...
def deadends_no_more(solution):
    ...
    for r in range(0, len(solution)):
        for c in range(0, len(solution[0])):
            if (solution[r][c] != '#') or (solution[r][c] != 'p') or (solution[r][c] != '!'):
                path_length += 1
            if solution[r][c] == '!':
                solution[r][c] = 'p'
...
print(printmaze(revert(final_maze))) #printing final_maze as a list of strings
print('The length of the minimum path =', min_path)

# We still have to prompt an input for the maze.

# Also, a None value prints on output which I think is coming from deadends_no_more. Gotta find a way to remove that.